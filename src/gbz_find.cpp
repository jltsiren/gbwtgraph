...
/*
  ...
  outputs three tab-separated fields:
  ...
  TODO: Reference coordinates if the selected reference is one of the paths?
*/
  ...
  progress(false), threads(default_threads())
{
  ...
  // Process options.
  while((c = getopt_long(argc, argv, "t:ph", long_options, &option_index)) != -1)
  {
    switch(c)
    {
    case 't':
