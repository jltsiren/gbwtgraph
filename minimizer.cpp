...
void
MinimizerIndex::Header::sanitize()
{
...
void
MinimizerIndex::swap(MinimizerIndex& another)
{
                     ...
                     const sdsl::bit_vector& is_pointer, bool& ok)
{
  ...
  for(size_t i = 0; i < hash_table.size(); i += BLOCK_SIZE)
  {
    ...
    {
      if(is_pointer[i + j]) { buffer[j].second.value = MinimizerIndex::NO_VALUE; }
    }
...
MinimizerIndex::serialize(std::ostream& out) const
{
  size_t bytes = 0;
  bool ok = true;
  ...
  {
    if(this->is_pointer[i]) { bytes += mi::serialize_vector(out, *(this->hash_table[i].second.pointer), ok); }
  }
...
bool
MinimizerIndex::operator==(const MinimizerIndex& another) const
{
...
void
MinimizerIndex::copy(const MinimizerIndex& source)
{
  this->clear();
...
{
  for(size_t i = 0; i < this->hash_table.size(); i++) { this->clear(i); }
}
...
/*
  ...
  by both key and position. The candidate at position j is removed when we reach position
  j + w. Candidates at the tail are purged when we advance with a smaller key.
*/
struct CircularBuffer
{
  ...
  void advance(offset_type pos, key_type forward_key, key_type reverse_key)
  {
    ...
    size_t forward_hash = gbwt::wang_hash_64(forward_key), reverse_hash = gbwt::wang_hash_64(reverse_key);
...
update_forward_key(MinimizerIndex::key_type& key, size_t k, unsigned char c, size_t& valid_chars)
{
  ...
  if(packed > MinimizerIndex::PACK_MASK) { key = 0; valid_chars = 0; }
  else
  {
...
{
  MinimizerIndex::key_type packed = MinimizerIndex::CHAR_TO_PACK[c];
  if(packed > MinimizerIndex::PACK_MASK) { key = 0; }
  else
  {
...
void
MinimizerIndex::insert(const minimizer_type& minimizer, const pos_t& pos)
{
...
std::vector<pos_t>
MinimizerIndex::find(const minimizer_type& minimizer) const
{
...
size_t
MinimizerIndex::count(const minimizer_type& minimizer) const
{
...
size_t
MinimizerIndex::find_offset(key_type key, size_t hash) const
{
...
void
MinimizerIndex::insert(key_type key, code_type pos, size_t offset)
{
...
void
MinimizerIndex::append(code_type pos, size_t offset)
{
...
bool
MinimizerIndex::contains(size_t offset, code_type pos) const
{
  if(this->is_pointer[offset])
  {
...
MinimizerIndex::rehash()
{
  ...
  for(size_t i = 0; i < old_hash_table.size(); i++)
  {
    ...
    size_t offset = this->find_offset(key, gbwt::wang_hash_64(key));
...
void
index_haplotypes(const GBWTGraph& graph, MinimizerIndex& index)
{
