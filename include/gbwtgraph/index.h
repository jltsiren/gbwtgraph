                      ...
                      const std::function<PayloadType(const pos_t&)>& get_payload)
{
    ...
    auto path_ids_map = gbwtgraph::detail::PathIDMap(meta);

    // Define cache element type based on payload type
    using TraversedNodes = std::conditional_t<
        std::is_same<PayloadType, PayloadXL>::value,
        std::vector<gbwt::node_type>,
        std::monostate>;

    struct alignas(CACHE_LINE_SIZE) Cache {
        std::vector<std::tuple<minimizer_type, pos_t, TraversedNodes>> data;
    };
    ...
    // Lambda for minimizer extraction without path tracking
    auto find_minimizers = [&](const std::vector<handle_t>& traversal, const std::string& seq) {
        ...
        for (minimizer_type& minimizer : minimizers) {
            ...
            if (!Position::valid_offset(pos)) {
                ...
                std::cerr << "index_haplotypes(): Invalid node offset " << offset(pos) << std::endl;
                std::exit(EXIT_FAILURE);
            }

            cache[thread_id].data.emplace_back(minimizer, pos, TraversedNodes{});
        }

        if (cache[thread_id].data.size() >= MINIMIZER_CACHE_SIZE) flush_cache(thread_id);
    };

    // Lambda for minimizer + path tracking (only used with PayloadXL)
    auto find_minimizers_and_paths = [&](const std::vector<handle_t>& traversal, const std::string& seq) {
        std::vector<minimizer_type> minimizers = index.minimizers(seq);
        auto iter = traversal.begin();
        size_t node_start = 0;
        int thread_id = omp_get_thread_num();
        ...
        for (minimizer_type& minimizer : minimizers) {
            ...
            if (minimizer.is_reverse) {
                extend([&] { if (node_iter == traversal.begin()) return false; --node_iter; return true; });
                std::reverse(traversed_nodes.begin(), traversed_nodes.end());
            } else {
                extend([&] { if (std::next(node_iter) == traversal.end()) return false; ++node_iter; return true; });
            }
    ...
    if constexpr (std::is_same<PayloadType, PayloadXL>::value) {
        for_each_haplotype_window(graph, index.window_bp(), find_minimizers_and_paths, threads > 1);
    } else {
