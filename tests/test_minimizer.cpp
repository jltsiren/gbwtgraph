...
TEST(ObjectManipulation, EmptyIndex)
{
  ...
  EXPECT_EQ(default_index, default_copy);
  EXPECT_EQ(alt_index, alt_copy);
  EXPECT_NE(default_index, alt_index);
}
...
TEST(ObjectManipulation, Contents)
{
  ...
  EXPECT_NE(default_index, default_copy);
...
TEST(ObjectManipulation, Swap)
{
  ...
  first.swap(second);
  EXPECT_NE(first, first_copy);
  EXPECT_EQ(first, second_copy);
  EXPECT_EQ(second, first_copy);
  EXPECT_NE(second, second_copy);
}
...
TEST_F(MinimizerExtraction, BothOrientations)
{
  ...
  ASSERT_EQ(forward_minimizers.size(), reverse_minimizers.size());
  for(size_t i = 0; i < forward_minimizers.size(); i++)
  {
    ...
    EXPECT_EQ(f.key, r.key);
    EXPECT_EQ(f.offset, this->str.length() - 1 - r.offset);
    EXPECT_NE(f.is_reverse, r.is_reverse);
  }
}
...
public:
  ...
  {
    ASSERT_EQ(index.size(), keys);
    ASSERT_EQ(index.values(), values);
    EXPECT_EQ(index.unique_keys(), unique);
...
TEST_F(CorrectKmers, MissingKeys)
{
  ...
  {
    EXPECT_TRUE(index.find(get_minimizer(i)).empty());
  }
}
...
TEST_F(CorrectKmers, EmptyKeysValues)
{
  ...
  EXPECT_TRUE(index.find(get_minimizer(MinimizerIndex::NO_KEY)).empty());

  index.insert(get_minimizer(this->total_keys + 1), MinimizerIndex::decode(MinimizerIndex::NO_VALUE));
  EXPECT_TRUE(index.find(get_minimizer(this->total_keys + 1)).empty());
}
...
TEST_F(CorrectKmers, Rehashing)
{
  ...
  ASSERT_EQ(index.max_keys(), threshold);
  ...
  EXPECT_GT(index.max_keys(), threshold);
